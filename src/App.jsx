  ...
  const determineStage = (months) => {
    if (months < 4) {
      return {
        stageId: "none",
        stageName: "이유식 준비기",
        description: "모유·분유만으로 충분한 시기입니다.",
        bullets: [
          "아기의 소화기관이 아직 이유식을 받아들일 준비가 되지 않았어요",
          "목 가누기가 되고, 음식에 관심을 보이며, 숟가락을 밀어내지 않을 때가 이유식 시작 신호입니다",
          "이 시기에는 수유량을 충분히 채워주는 것이 중요합니다",
          "분유에 미음을 섞거나 억지로 먹이는 행동은 피해주세요",
        ],
      };
    } else if (months <= 6) {
      return {
        stageId: "early",
        stageName: "초기",
        description: "부드러운 10배죽(쌀미음)으로 가볍게 시작하는 단계입니다.",
        bullets: [
          "한 가지 재료를 3~4일 간격으로 소량씩 추가하며 알레르기 여부(발진·구토·설사)를 면밀히 살핍니다",
          "6개월부터는 철분 보충을 위해 쇠고기를 포함한 육류 섭취가 필수적입니다",
          "오전 수유 후 1~2숟가락으로 시작해 서서히 양을 늘려가세요",
          "초기 1단계(4~5개월): 단일 재료 미음 / 초기 2단계(5~6개월): 2~3가지 재료 조합",
          "절대 피해야 할 식재료: 꿀, 생우유, 날달걀",
          "목표 섭취량: 한 번에 60~80ml 정도",
        ],
      };
    } else if (months <= 9) {
      return {
        stageId: "middle",
        stageName: "중기",
        description: "두부 정도 굳기의 진죽(7~8배죽)을 하루 2회 먹는 시기입니다.",
        bullets: [
          "잇몸으로 오물오물 음식을 으깨는 훈련이 활발하게 일어납니다",
          "매일 소고기 또는 닭고기를 규칙적으로 식단에 포함시켜 철분을 보충해 주세요",
          "당근·브로콜리 등 채소 재료를 다변화하고 수유와 이유식을 분리해 식사 습관을 잡아줍니다",
          "자기주도 이유식(핑거푸드)을 시작하기 좋은 시기예요",
          "하루 목표 섭취량: 80~120ml × 2회",
          "7개월 이후 간식을 하루 1회 도입할 수 있어요",
        ],
      };
    } else if (months <= 11) {
      return {
        stageId: "late",
        stageName: "후기",
        description: "바나나 정도 질감의 무른밥을 하루 3회 정해진 시간에 먹기 시작합니다.",
        bullets: [
          "밥+국+반찬처럼 어른 식탁과 유사한 구성으로 다진 식재료를 제공하세요",
          "숟가락 연습과 핑거푸드로 스스로 집어먹는 자율성과 소근육 발달을 유도해 주세요",
          "어른 음식은 간하기 전에 따로 덜어주고, 아기 음식엔 간을 하지 않습니다",
          "다양한 식재료 경험으로 편식을 예방하는 시기입니다",
          "하루 목표 섭취량: 120~150ml × 3회",
          "야간 수유를 서서히 줄여가는 시기입니다",
        ],
      };
    } else if (months <= 23) {
      return {
        stageId: "complete",
        stageName: "완료기",
        description: "진밥·잡곡밥을 기본으로 하루 세 끼 건강한 밥상을 완성하는 단계입니다.",
        bullets: [
          "식판식으로 다양한 영양소를 고루 섭취하고 편식 습관을 사전에 예방하세요",
          "아기용 저염 조미료로 어른 간의 1/3 이하 가벼운 저염식을 시작할 수 있습니다",
          "숟가락·포크 사용을 자연스럽게 격려하며 자립 식사 습관을 만들어 주세요",
          "가족과 함께 식사하며 올바른 식습관의 기초를 다져줍니다",
          "만 12개월부터 분유 → 생우유 전환이 가능합니다",
          "하루 유제품 목표량: 400~500ml (우유·치즈·요거트 포함)",
        ],
      };
    } else {
      return {
        stageId: "graduation",
        stageName: "이유식 졸업",
        description: "성인 식사와 유사한 유아식으로 전환하는 완성기입니다.",
        bullets: [
          "맵고 짠 자극적인 음식은 여전히 삼가야 하며 저당·저염 원칙을 장기적으로 지켜주세요",
          "편식을 교정하기 위해 거부하는 재료는 조리 형태를 바꾸어(다지거나 전으로 굽기) 친밀해지도록 해보세요",
          "하루 세 끼 규칙적인 식사 습관을 유지하는 것이 중요합니다",
          "생우유로 완전 전환이 완료되는 시기입니다",
          "견과류 등 질식 위험 식재료는 잘게 다지거나 으깨서 제공하세요",
        ],
      };
    }
  };
                    ...
                    <div className="babyDateResult">
                      <div className="babyDateResultContent">
                        ...
                        <div className="babyDateResultText">
                          ...
                          {/* bullets 배열이 있으면 목록으로, 없으면 단락으로 표시합니다 */}
                          {Array.isArray(babyStageInfo.bullets) ? (
                            <ul className="resultSubList">
                              {babyStageInfo.bullets.map((item, idx) => (
                                <li key={idx}>{item}</li>
                              ))}
                            </ul>
                          ) : (
